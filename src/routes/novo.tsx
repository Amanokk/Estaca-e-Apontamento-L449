...
export const Route = createFileRoute("/novo")({
  validateSearch: (raw: Record<string, unknown>): { edit?: string; estaca?: string; street?: string } => {
    const out: { edit?: string; estaca?: string; street?: string } = {};
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      if (streetFromMap && data?.streets) {
        const hit =
          data.streets.find((s) => s.name === streetFromMap) ??
          matchStreetByLabel(streetFromMap, data.streets, next.workId);
        if (hit) {
