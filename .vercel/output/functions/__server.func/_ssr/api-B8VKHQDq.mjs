...
var isVercel = typeof process !== "undefined" && Boolean(process.env.VERCEL);
/**
...
async function createSql() {
	...
	if (isVercel) throw new Error("NO_DATABASE");
...
function ensureDbReady() {
	if (dbSource !== "pglite" || isVercel) return Promise.resolve();
	return getSql().then(() => void 0);
}
var globalBoot = globalThis;
if (typeof window === "undefined" && dbSource === "pglite" && !isVercel) globalBoot.__pgBootstrapPromise__ ??= ensureDbReady().catch((err) => {
